...
impl Cpu {
    ...
    /// Returns the EXTRA cycles to add on top of the base instruction cycles.
    /// For loads: base = 3 (1S + 1N + 1I). The N cycle already includes
    /// the memory access time, so we only add the difference between
    /// the N cycle and 1 (the default N=1 for IWRAM).
    #[inline]
