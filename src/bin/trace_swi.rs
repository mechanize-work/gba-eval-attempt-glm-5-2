use std::fs;
fn main() {
    let rom_data = fs::read("dev-roms/anguna.gba").expect("Failed to read ROM");
    gba_emu::emulator::init();
    let rom_ptr = gba_emu::emulator::rom_buffer_ptr();
    unsafe {
        let rom_slice = std::slice::from_raw_parts_mut(rom_ptr, rom_data.len());
        rom_slice.copy_from_slice(&rom_data);
    }
    gba_emu::emulator::load_rom(rom_data.len());
    let emu = gba_emu::emulator::get_emu();
    
    for i in 0..500000u32 {
        let pc = emu.cpu.r[15];
        let thumb = emu.cpu.is_thumb();
        let instr: u32 = if thumb { emu.mem.read_half(pc) as u32 } else { emu.mem.read_word(pc) };
        
        // Check for SWI instruction
        if thumb && (instr & 0xFF00) == 0xDF00 {
            eprintln!("[{}] SWI {} at PC=0x{:08X} (THUMB) R0={:08X} R1={:08X} R2={:08X} R3={:08X}",
                i, instr & 0xFF, pc, emu.cpu.r[0], emu.cpu.r[1], emu.cpu.r[2], emu.cpu.r[3]);
        } else if !thumb && (instr >> 24) == 0xEF {
            eprintln!("[{}] SWI 0x{:06X} at PC=0x{:08X} (ARM)", i, instr & 0xFFFFFF, pc);
        }
        
        gba_emu::emulator::step_one();
        
        if i % 100000 == 0 && i > 0 {
            eprintln!("[{}] Still running... PC=0x{:08X}", i, emu.cpu.r[15]);
        }
    }
}
