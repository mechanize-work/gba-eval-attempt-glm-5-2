...
fn main() {
    ...
    for frame in 0..500 {
        ...
        if dispcnt != 0x0080 || halted {
            let ime = (emu.mem.io[0x208] as u16) | ((emu.mem.io[0x209] as u16) << 8);
            let ie = (emu.mem.io[0x200] as u16) | ((emu.mem.io[0x201] as u16) << 8);
            eprintln!("Frame {}: PC=0x{:08X} DISPCNT=0x{:04X} halted={} IME={} IE=0x{:04X}",
                frame, pc, dispcnt, halted, ime, ie);
        }
        
        if dispcnt != 0x0080 && dispcnt != 0x0000 {
            eprintln!("  DISPLAY ACTIVE! Breaking.");
            break;
        }
        
        if halted {
            eprintln!("  CPU halted! Breaking.");
            break;
        }
    }
}
