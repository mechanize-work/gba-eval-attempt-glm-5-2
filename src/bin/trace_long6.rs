use std::fs;
fn main() {
    let rom_data = fs::read("dev-roms/anguna.gba").expect("Failed to read ROM");
    gba_emu::emulator::init();
    let rom_ptr = gba_emu::emulator::rom_buffer_ptr();
    unsafe {
        let rom_slice = std::slice::from_raw_parts_mut(rom_ptr, rom_data.len());
        rom_slice.copy_from_slice(&rom_data);
    }
    gba_emu::emulator::load_rom(rom_data.len());
    let emu = gba_emu::emulator::get_emu();
    
    // Run 5 frames (enough for init + VBlank wake-up + init code execution)
    for f in 0..5 {
        gba_emu::emulator::run_frame();
        let dc = (emu.mem.io[0x00] as u16) | ((emu.mem.io[0x01] as u16) << 8);
        eprintln!("Frame {}: DC=0x{:04X} halted={} PC=0x{:08X} [15E0]=0x{:08X} VB_h=0x{:08X}",
            f, dc, emu.cpu.halted, emu.cpu.r[15],
            emu.mem.read_word(0x030015E0), emu.mem.read_word(0x03003E5C));
    }
    
    // Step through the halt and wake-up
    let mut last_15e0 = emu.mem.read_word(0x030015E0);
    let mut last_dc = (emu.mem.io[0x00] as u16) | ((emu.mem.io[0x01] as u16) << 8);
    let mut last_vb = emu.mem.read_word(0x03003E5C);
    
    for i in 0..500000u64 {
        gba_emu::emulator::step_one();
        
        let v15e0 = emu.mem.read_word(0x030015E0);
        let dc = (emu.mem.io[0x00] as u16) | ((emu.mem.io[0x01] as u16) << 8);
        let vb = emu.mem.read_word(0x03003E5C);
        
        if v15e0 != last_15e0 || dc != last_dc || vb != last_vb {
            eprintln!("[{}] [15E0]=0x{:08X} DC=0x{:04X} VB_h=0x{:08X} PC=0x{:08X} halted={}",
                i, v15e0, dc, vb, emu.cpu.r[15], emu.cpu.halted);
            last_15e0 = v15e0;
            last_dc = dc;
            last_vb = vb;
        }
        
        if dc != 0x0080 && dc != 0x0000 {
            eprintln!("DISPLAY ACTIVE at step {}!", i);
            break;
        }
    }
}
