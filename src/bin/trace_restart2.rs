...
fn main() {
    ...
    // Step to 204650 (just before BSS clear finishes)
    for i in 0..204650 { gba_emu::emulator::step_one(); }
    
    // Trace 300 instructions, tracking when PC changes significantly
    ...
    for j in 0..500u32 {
        ...
        let range = pc & 0xFFFFF000;
        if range != last_range || is_swi || pc < 0x04000000 || pc == 0 {
            let swi_info = if is_swi {
                if thumb { format!(" SWI={}", instr & 0xFF) }
                else { format!(" ARM_SWI=0x{:X}", instr & 0xFFFFFF) }
            } else { String::new() };
            eprintln!("[{}] PC=0x{:08X} 0x{:X} {} {} DC=0x{:04X} LR=0x{:08X} R0={:08X} R1={:08X}{}",
                j, pc, instr, mode_str, if thumb {"T"} else {"A"}, dc, lr,
                emu.cpu.r[0], emu.cpu.r[1], swi_info);
            last_range = range;
        }
        
        gba_emu::emulator::step_one();
        
        if emu.cpu.r[15] == 0x08000190 && j > 10 {
            eprintln!("[{}] *** RESTARTED ***", j+1);
            break;
        }
    }
}
