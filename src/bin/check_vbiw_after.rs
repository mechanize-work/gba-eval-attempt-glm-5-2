use std::fs;
fn main() {
    let rom_data = fs::read("dev-roms/anguna.gba").expect("Failed to read ROM");
    gba_emu::emulator::init();
    let rom_ptr = gba_emu::emulator::rom_buffer_ptr();
    unsafe {
        let rom_slice = std::slice::from_raw_parts_mut(rom_ptr, rom_data.len());
        rom_slice.copy_from_slice(&rom_data);
    }
    gba_emu::emulator::load_rom(rom_data.len());
    let emu = gba_emu::emulator::get_emu();
    
    for f in 0..3 { gba_emu::emulator::run_frame(); }
    
    eprintln!("After 3 rf: halted={} vbiw={} vb_occ={} [15E0]={} cc={} scan={} cis={}",
        emu.cpu.halted, emu.cpu.vblank_intr_wait, emu.vblank_occurred,
        emu.mem.read_word(0x030015E0), emu.cycle_count,
        emu.current_scanline, emu.cycle_in_scanline);
    
    // Check: is cycle_in_scanline the issue?
    // If cycle_in_scanline is not 0, the first scanline advance might not
    // happen immediately, delaying VBlank.
    // cycle_in_scanline should be small (2 from BIOS).
    // 1232 - 2 = 1230 cycles until first scanline change.
    // 160 * 1232 = 197120 cycles until VBlank.
    // 2 + 197120 = 197122 < 280898. So VBlank should fire.
    
    // Let me just run rf4 and track scanline
    let target = emu.cycle_count.wrapping_add(280896);
    let mut count = 0u64;
    let mut last_scan = emu.current_scanline;
    let mut last_15e0 = emu.mem.read_word(0x030015E0);
    
    while emu.cycle_count < target && count < 500000 {
        emu.check_and_handle_interrupts();
        
        if emu.cpu.halted {
            emu.cycle_count = emu.cycle_count.wrapping_add(1);
            emu.advance_hardware(1);
        } else {
            emu.execute_one();
            emu.check_and_handle_interrupts();
        }
        count += 1;
        
        if emu.current_scanline != last_scan {
            if emu.current_scanline == 160 {
                eprintln!("[{}] VBlank! scan=160 vb_occ={} halted={} vbiw={} [15E0]={}",
                    count, emu.vblank_occurred, emu.cpu.halted,
                    emu.cpu.vblank_intr_wait, emu.mem.read_word(0x030015E0));
            }
            last_scan = emu.current_scanline;
        }
        
        let v = emu.mem.read_word(0x030015E0);
        if v != last_15e0 {
            eprintln!("[{}] [15E0] {}->{} PC=0x{:08X}", count, last_15e0, v, emu.cpu.r[15]);
            last_15e0 = v;
        }
    }
    
    eprintln!("After: halted={} [15E0]={} count={}", emu.cpu.halted, last_15e0, count);
}
